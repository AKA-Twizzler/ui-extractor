...
def window_groups(m):
    ...
    if rest and groups:
        ...
        # Finder came back as four narrow panes - `Maci`, `ntoshHD>`,
        # `Users`, `jaredrhode`, `claude>` - and none was claimed, so the
        # window never got a path and was drawn with no name at all. The
        # pieces are taken together, not one at a time: panes standing in ONE
        # row, holding mostly crumbs, lying inside a window's own width and
        # against its foot, are that window's bar. A window that already has
        # a crumb-bearing pane is left alone, so this can only ever supply a
        # bar that is missing.
        crumby = []
        for p in still:
