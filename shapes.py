...
def scroll_thumb(path, rect, reach=None, stop=None):
    ...
    (top_share, height_share) of the pane's height, or None where the pane
    shows no thumb (everything in view, or the bar hidden). The reader
    never reads a scroll bar as text; this is the one way the note knows a
    window held more than it showed."""
    try:
    ...
    if k < h:
        lit[k:, :] = False
    col = lit.mean(axis=0)
    cols = [int(c) for c in np.where((col > 0.04) & (col < 0.95))[0]]
    if not cols:
        return None
    runs, cur = [], [cols[0]]
    for c in cols[1:]:
