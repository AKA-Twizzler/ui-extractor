...
def unzoomed_by_bar(frame):
    ...
    # THE BAR'S WRITING IS BRIGHTER THAN ITS GROUND, and that is what tells it
    # from any other full-width strip. The test used to ask only that pixels
    # DIFFER from the median, and a browser's tab bar with the black desktop
    # showing beside it differs just as much -- so 00:04:00, plainly zoomed
    # into an Obsidian window under a browser, was called unzoomed and the
    # whole desktop drawn at 1.00. Measured over this video's frames:
    #
    #     a real menu bar      darker 0.000   brighter 0.044 to 0.059
    #     a browser tab strip  darker 0.086   brighter 0.002
    #
    # Writing on a dark bar is bright; a block of desktop is dark. So only the
    # brighter pixels count as the bar's own ink, and a strip whose difference
    # is mostly darkness is not a menu bar.
    diff = strip.astype(np.int16) - med
    ink = diff > 40
    if ink.mean() < 0.005 or (diff < -40).mean() > ink.mean():
        return None
    # and the ink must reach both ends: a bar spans the screen, a window's
    # toolbar does not
    on = np.flatnonzero(ink.any(axis=0))
    if on.size == 0 or on[0] > 0.06 * w or on[-1] < 0.94 * w:
        return None
    return 1.0
